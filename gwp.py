#Module to use system commands on windows
import subprocess

#Module to use regex
import re

#We can run system commands using the function provided by the subprocess module:
...
#    We create an empty list where dictionaries containing all the wifi usernames and passwords will be saved.
wifi_list = []

#   Check if there is at least a wifi profile saved
if len(profile_names) != 0:
    for name in profile_names:
        ...
        if re.search("Security key           : Absent", profile_info):
            ...
            #   Now we should run the "key=clear" command part to get the password.
